...
public class VanyaAndFoodProcessor {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; ++i) {
            potatoes[i] = in.readInt();
        }

        out.print(solve(n, h, k, potatoes));
        out.close();
    }

    private static long solve(int n, long h, long k, int[] potatoes) {
        int pointer = 0;
        long count = 0;
        long result = 0;
        ...
        while(pointer < n) {
            ...
            if (count + potatoes[pointer] <= h) {
                count += potatoes[pointer];
            } else {
                result += 1;
                count = potatoes[pointer];
            }
