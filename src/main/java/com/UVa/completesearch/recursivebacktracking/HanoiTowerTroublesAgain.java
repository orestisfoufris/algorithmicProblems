...
/**
 * https://uva.onlinejudge.org/external/102/10276.pdf
 */
public class HanoiTowerTroublesAgain {
    ...
    public static void main(String[] args) {
        ...
        while (T-- > 0) {
            ...
            for (int i = 0; ; ++i) {
                boolean placed = solve(list, 0, list.size(), i + 1);
                if (!placed) break;
            }
    ...
    private static boolean solve(List<Deque<Integer>> list, int start, int end, int toPlace) {

       if (start < end) {
           if (!list.get(start).isEmpty()) {
               Integer current = list.get(start).peek();
               if (isPerfectSquare(current + toPlace)) {
                   list.get(start).push(toPlace);
               } else {
                   return solve(list, start + 1, end, toPlace);
               }
